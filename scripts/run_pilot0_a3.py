#!/usr/bin/env python3
"""Execute frozen Pilot 0 A3 randomized interface-form bundle branches.

A3 crosses legacy-form (J_L) versus reconstructed-form (J_R) presentation with
identical E0/EV signal text. Eight randomized branches are required per frozen
prestate, exactly two per factorial cell. Branch id fixes the generation seed;
interface and signal labels never enter the seed function.
"""
...
def validate_frozen_prestate(row: dict, pilot_config: dict, model_dir: Path) -> None:
    required = {
        ...
        "question", "options", "benchmark_answer", "initial_answer", "p_correct", "i",
...
def validate_design(assignments: list[dict], frozen: dict[str, dict], config: dict) -> None:
    ...
    for branch in assignments:
        ...
        if cell not in CELLS:
            raise ValueError(f"branch {unit_id}: invalid A3 cell {cell!r}")
        ...
        if bool(source["initial_correct"]):
            raise ValueError(f"branch {unit_id}: A3 includes initially-correct prestate")
...
def legacy_form_message(source: dict, signal_text: str, revision_instruction: str) -> str:
    question = render_question(str(source["question"]), [str(x) for x in source["options"]])
    previous = (
        f"Your previous answer was {source['initial_answer']}. "
...
def reconstructed_message(signal_text: str, revision_instruction: str) -> str:
    return signal_text + "\n\n" + revision_instruction


def build_messages(source: dict, interface: str, signal_text: str, revision_instruction: str) -> tuple[list[dict], str]:
    if interface == "J_L":
        user_message = legacy_form_message(source, signal_text, revision_instruction)
        return [
            {"role": "user", "content": user_message},
            {"role": "assistant", "content": ASSISTANT_PREFILL},
        ], user_message
    if interface == "J_R":
        user_message = reconstructed_message(signal_text, revision_instruction)
        return [
            {"role": "user", "content": str(source["pre_prompt"])},
            {"role": "assistant", "content": str(source["raw_model_output"])},
            {"role": "user", "content": user_message},
            {"role": "assistant", "content": ASSISTANT_PREFILL},
        ], user_message
...
def main() -> int:
    ...
    parser.add_argument("--a3-config", type=Path, default=Path("experiments/PILOT0_A3_CONFIG.json"))
    ...
    config = load_json(args.a3_config)
    if config.get("status") != "frozen before any A3 outcome generation":
        raise ValueError("A3 contract is not in the expected frozen pre-outcome state")
    ...
    for field, expected in frozen_values.items():
        if not same_float(inv[field], expected):
            raise ValueError(f"A3 invariant {field} differs from frozen Pilot 0 generation")
    if inv["assistant_prefill"] != ASSISTANT_PREFILL or inv["choice_constraint"] != CHOICE_CONSTRAINT:
        raise ValueError("A3 output interface differs from frozen Pilot 0 answer interface")
    ...
    for index, branch in enumerate(assignments, 1):
        ...
        messages, user_message = build_messages(source, interface, signal_text, revision_instruction)
        valid_letters = letters_for([str(x) for x in source["options"]])
        base_seed = stable_seed(int(inv["generation_base_seed"]), f"a3::{branch['id']}")
        ...
        out.update({
            ...
            "post_seed_rule": "stable_seed(generation_base_seed, 'a3::' + branch_id)",
    ...
    print(f"wrote {len(output)} A3 outcome branches", file=sys.stderr)
