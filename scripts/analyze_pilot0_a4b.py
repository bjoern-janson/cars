#!/usr/bin/env python3
"""Analyze frozen Pilot 0 A4b historical-prestate rendering localization.

Primary scientific unit: one initially-wrong frozen prestate block.
Primary endpoints:
  Delta_H_change       = T_change(H_verbatim,E0) - T_change(H_canonical,E0)
  Delta_H_instability  = T_instability(H_verbatim,E0) - T_instability(H_canonical,E0)
  Delta_H_verified     = T_verified(H_verbatim,EV) - T_verified(H_canonical,EV)

The only primary p-values are two-sided blocked randomization p-values for those
three effects, Holm-adjusted as one family. Q_N and Gamma_HE are secondary.
"""
...
PRIMARY = ("Delta_H_change", "Delta_H_instability", "Delta_H_verified")
...
def validate_rows(rows: Sequence[dict], config: dict) -> dict[str, list[dict]]:
    ...
    for row in rows:
        ...
        cell = str(row.get("a4b_cell") or row.get("arm") or "")
        if cell not in CELLS or str(row.get("arm")) != cell:
            raise ValueError(f"branch {row.get('id')}: invalid/disagreeing A4b cell")
        ...
        expected_base = stable_seed(base_seed, f"a4b::{row['id']}")
    ...
    for task_id, block_rows in grouped.items():
        ...
        counts = Counter(str(row["a4b_cell"]) for row in block_rows)
        ...
        if next(iter(initial)) == next(iter(benchmark)):
            raise ValueError(f"block {task_id}: A4b contains initially-correct prestate")

        # The signal/revision turn is invariant across rendering within each signal.
        for signal in ("E0", "EV"):
            signal_rows = [row for row in block_rows if str(row["signal"]) == signal]
            signal_hashes = {str(row.get("signal_revision_sha256", "")) for row in signal_rows}
            signal_turns = {str(row.get("signal_revision_turn", "")) for row in signal_rows}
            if len(signal_hashes) != 1 or len(signal_turns) != 1:
                raise ValueError(f"block {task_id} signal {signal}: rendering changed signal/revision turn")

        # Prestate rendering is constant within rendering arm and may differ only across arms.
        for rendering in ("H_canonical", "H_verbatim"):
            rr = [row for row in block_rows if str(row["rendering"]) == rendering]
            user_hashes = {str(row.get("user_prestate_sha256", "")) for row in rr}
            assistant_hashes = {str(row.get("assistant_prestate_sha256", "")) for row in rr}
            if len(user_hashes) != 1 or len(assistant_hashes) != 1:
                raise ValueError(f"block {task_id} {rendering}: prestate rendering changed across branches")

        canonical_rows = [row for row in block_rows if str(row["rendering"]) == "H_canonical"]
        verbatim_rows = [row for row in block_rows if str(row["rendering"]) == "H_verbatim"]
        if any(
            str(row.get("user_prestate_sha256", "")) != str(row.get("canonical_user_prestate_sha256", ""))
            or str(row.get("assistant_prestate_sha256", "")) != str(row.get("canonical_assistant_prestate_sha256", ""))
            for row in canonical_rows
        ):
            raise ValueError(f"block {task_id}: H_canonical provenance mismatch")
        if any(
            str(row.get("user_prestate_sha256", "")) != str(row.get("historical_user_prestate_sha256", ""))
            or str(row.get("assistant_prestate_sha256", "")) != str(row.get("historical_assistant_prestate_sha256", ""))
            for row in verbatim_rows
        ):
            raise ValueError(f"block {task_id}: H_verbatim provenance mismatch")
...
def collapse_block(task_id: str, rows: Sequence[dict]) -> dict:
    ...
    by_cell = {cell: [row for row in rows if str(row["a4b_cell"]) == cell] for cell in CELLS}
    canonical = rendering_metrics(by_cell["HC_E0"], by_cell["HC_EV"], initial, benchmark)
    verbatim = rendering_metrics(by_cell["HV_E0"], by_cell["HV_EV"], initial, benchmark)
    tau_c = canonical["mean_V_EV"] - canonical["mean_V_E0"]
    tau_v = verbatim["mean_V_EV"] - verbatim["mean_V_E0"]
    return {
        "task_id": task_id,
        "H_canonical": canonical,
        "H_verbatim": verbatim,
        "Delta_H_change": verbatim["T_change"] - canonical["T_change"],
        "Delta_H_instability": verbatim["T_instability"] - canonical["T_instability"],
        "Delta_H_verified": verbatim["T_verified"] - canonical["T_verified"],
        "tau_E_canonical": tau_c,
        "tau_E_verbatim": tau_v,
        "Gamma_HE": tau_v - tau_c,
    }
...
def qn_summary(blocks: Sequence[dict]) -> dict:
    ...
    qc = out["H_canonical"]["Q_N"]
    qv = out["H_verbatim"]["Q_N"]
    out["difference_verbatim_minus_canonical"] = None if qc is None or qv is None else float(qv) - float(qc)
    return out


def secondary_estimates(blocks: Sequence[dict]) -> dict:
    tau_c = mean([float(block["tau_E_canonical"]) for block in blocks])
    tau_v = mean([float(block["tau_E_verbatim"]) for block in blocks])
    return {
        "tau_E_canonical": tau_c,
        "tau_E_verbatim": tau_v,
        "Gamma_HE": tau_v - tau_c,
        "G_verified_minus_neutral_canonical": tau_c,
        "G_verified_minus_neutral_verbatim": tau_v,
    }
...
def bootstrap(blocks: Sequence[dict], config: dict) -> dict:
    ...
    qn_adjudicable = (
        int(observed_qn["H_canonical"]["revision_events"]) >= threshold
        and int(observed_qn["H_verbatim"]["revision_events"]) >= threshold
    )
    ...
    for _ in range(n_boot):
        ...
        gamma_values.append(float(secondary_estimates(sample)["Gamma_HE"]))
        if qn_adjudicable:
            diff = qn_summary(sample)["difference_verbatim_minus_canonical"]
...
def permuted_metrics(rows: Sequence[dict], rng: random.Random, signal: str, initial: str, benchmark: str) -> tuple[dict, dict]:
    ...
    canonical, verbatim = shuffled[:2], shuffled[2:]
    if signal == "E0":
...
def randomization_pvalues(grouped: dict[str, list[dict]], observed: dict, config: dict) -> dict:
    ...
    for _ in range(n_perm):
        ...
        for rows in grouped.values():
            initial = str(rows[0]["initial_answer"]).strip().upper()
            benchmark = str(rows[0]["benchmark_answer"]).strip().upper()
            c0, v0 = permuted_metrics(rows, rng, "E0", initial, benchmark)
            cv, vv = permuted_metrics(rows, rng, "EV", initial, benchmark)
            values["Delta_H_change"].append(v0["T_change"] - c0["T_change"])
            values["Delta_H_instability"].append(v0["T_instability"] - c0["T_instability"])
            values["Delta_H_verified"].append(vv["T_verified"] - cv["T_verified"])
        for key in PRIMARY:
    ...
    return {
        ...
        "rule": "within each prestate block, condition on signal and shuffle H_canonical/H_verbatim labels preserving exact 2/2 within E0 and exact 2/2 within EV",
    }
...
def classify(observed: dict, boot: dict, ri: dict, config: dict) -> dict:
    ...
    if earned:
        status = "CAUSAL_HISTORICAL_PRESTATE_RENDERING_SIGNAL"
        reason = "At least one prespecified historical-prestate rendering effect survives Holm familywise adjustment."
    elif all(equivalence.values()):
        status = "HISTORICAL_PRESTATE_RENDERING_WEAKENED_AT_A2_SCALE"
    ...
    return {
        ...
        "authority": (
            "A causal signal earns historical-prestate rendering as a causal component only for the named transition dimensions."
            if earned else "No causal historical-prestate-rendering component is earned by this gate."
        ),
    }
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("experiments/PILOT0_A4B_CONFIG.json"))
    parser.add_argument("--json-out", type=Path)
    args = parser.parse_args()

    config = load_json(args.config)
    if config.get("status") != "frozen before any A4b outcome generation":
        raise ValueError("A4b config is not the frozen pre-outcome contract")
    ...
    qn_report = {
        **qn,
        ...
        "bootstrap_95_difference_verbatim_minus_canonical": (
    ...
    report = {
        "schema_version": 1,
        "study": config["study"],
        "status": "A4b randomized historical-prestate rendering localization under frozen pre-outcome contract",
        ...
        "secondary": {
            **secondary,
            "Gamma_HE_bootstrap_95": boot["Gamma_HE"],
            "Q_N": qn_report,
        },
        "A4b_gate": decision,
