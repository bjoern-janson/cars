#!/usr/bin/env python3
"""Execute frozen Pilot 0 A4c legacy-inline rendering branches.

A4c crosses canonical versus legacy-style inline rendering while holding
single-user-turn topology and E0/EV signal/revision semantics fixed. Eight
...
def validate_design(assignments: list[dict], frozen: dict[str, dict], config: dict) -> None:
    ...
    for branch in assignments:
        ...
        if cell not in CELLS:
            raise ValueError(f"branch {unit_id}: invalid A4c cell {cell!r}")
        ...
        if bool(source["initial_correct"]):
            raise ValueError(f"branch {unit_id}: A4c includes initially-correct prestate")
...
def semantic_state_sha256(state: dict) -> str:
    payload = json.dumps(state, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
    return sha256_text(payload)


def canonical_inline_message(source: dict, signal_text: str, revision_instruction: str) -> str:
    question = "QUESTION:\n" + render_question(
    ...
    return SEPARATOR.join([question, state, signal, revision])


def legacy_inline_message(source: dict, signal_text: str, revision_instruction: str) -> str:
    question = render_question(str(source["question"]), [str(x) for x in source["options"]])
    previous = (
    ...
    return SEPARATOR.join([question, previous, signal_text, revision_instruction])
...
def build_messages(
    ...
    canonical = canonical_inline_message(source, signal_text, revision_instruction)
    legacy = legacy_inline_message(source, signal_text, revision_instruction)
    if rendering == "L_canonical":
        user_message = canonical
    elif rendering == "L_legacy":
        user_message = legacy
    else:
    ...
    provenance = {
        ...
        "canonical_user_message_sha256": sha256_text(canonical),
        "legacy_user_message_sha256": sha256_text(legacy),
        "semantic_state_sha256": semantic_state_sha256(state),
        "semantic_state_json": json.dumps(state, sort_keys=True, ensure_ascii=False),
    }
    messages = [
...
def verify_rendering_bridge(source: dict, signals: dict, revision_instruction: str) -> None:
    rendered_question = render_question(str(source["question"]), [str(x) for x in source["options"]])
    for signal in ("E0", "EV"):
        signal_text = str(signals[signal])
        _, canonical = build_messages(source, "L_canonical", signal_text, revision_instruction)
        _, legacy = build_messages(source, "L_legacy", signal_text, revision_instruction)
        if canonical["semantic_state_sha256"] != legacy["semantic_state_sha256"]:
            raise ValueError(f"{source['id']} {signal}: rendering changed semantic-state signature")
        if canonical["semantic_state_json"] != legacy["semantic_state_json"]:
            raise ValueError(f"{source['id']} {signal}: rendering changed semantic-state payload")
        if canonical["user_message_sha256"] != canonical["canonical_user_message_sha256"]:
            raise ValueError(f"{source['id']} {signal}: L_canonical provenance mismatch")
        if legacy["user_message_sha256"] != legacy["legacy_user_message_sha256"]:
            raise ValueError(f"{source['id']} {signal}: L_legacy provenance mismatch")
        if rendered_question not in canonical["user_message"] or rendered_question not in legacy["user_message"]:
            raise ValueError(f"{source['id']} {signal}: question/options differ across inline renderings")
        if signal_text not in canonical["user_message"] or signal_text not in legacy["user_message"]:
            raise ValueError(f"{source['id']} {signal}: signal differs across inline renderings")
        if revision_instruction not in canonical["user_message"] or revision_instruction not in legacy["user_message"]:
            raise ValueError(f"{source['id']} {signal}: revision instruction differs across inline renderings")
...
def main() -> int:
    ...
    parser.add_argument("--a4c-config", type=Path, default=Path("experiments/PILOT0_A4C_CONFIG.json"))
    ...
    config = load_json(args.a4c_config)
    if config.get("status") != "frozen before any A4c outcome generation":
        raise ValueError("A4c contract is not in the expected frozen pre-outcome state")
    ...
    for field, expected in frozen_values.items():
        if not same_float(inv[field], expected):
            raise ValueError(f"A4c invariant {field} differs from frozen Pilot 0 generation")
    if inv["assistant_prefill"] != ASSISTANT_PREFILL or inv["choice_constraint"] != CHOICE_CONSTRAINT:
        raise ValueError("A4c output interface differs from frozen Pilot 0 answer interface")
    ...
    for index, branch in enumerate(assignments, 1):
        ...
        base_seed = stable_seed(int(inv["generation_base_seed"]), f"a4c::{branch['id']}")
        ...
        out.update({
            ...
            **provenance,
            ...
            "post_seed_rule": "stable_seed(generation_base_seed, 'a4c::' + branch_id)",
    ...
    print(f"wrote {len(output)} A4c outcome branches", file=sys.stderr)
