...
def evaluate_system(system, cfg, seed):
    if not cfg["sampling"].get("pair_initial_states_across_systems", False):
        raise RuntimeError("Match1 requires pair_initial_states_across_systems=true")

    # Identical random streams across F/N/A guarantee identical realized x0/theta0
    # for every sequence and split. The system law is the only generator difference.
    rng_train = np.random.default_rng(seed + 11)
    rng_test = np.random.default_rng(seed + 111)
            ...
            out.append(
                {
                    ...
                    "train_samples": key[1],
                    "A_minus_N_structural_nmse": systems["A"]["structural_nmse"] - systems["N"]["structural_nmse"],
                    "A_minus_F_structural_nmse": systems["A"]["structural_nmse"] - systems["F"]["structural_nmse"],
                    "N_minus_F_structural_nmse": systems["N"]["structural_nmse"] - systems["F"]["structural_nmse"],
                }
            )
    return out
...
def main():
    ...
    result = {
        "schema_version": 1,
        "study": cfg["study"],
        "status": "SMOKE_ONLY" if args.smoke else "PILOT1_MATCH1_SYNTHETIC_OUTCOME",
        "authority": cfg["authority"],
        "rows": rows,
        "contrasts": contrasts,
        "primary_decision": decision,
        "guardrails": [
            "Match1 changes only realized initial-state pairing across F/N/A relative to Run 1.",
    ...
    print(json.dumps({"status": result["status"], "rows": len(rows), "primary_decision": decision, "out": args.json_out}, indent=2))
