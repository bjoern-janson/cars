#!/usr/bin/env python3
"""Execute frozen Pilot 0 A7 encoding x section-scaffold factorial branches.

A7 crosses prior-state encoding (fields/prose) with section scaffold
(labeled/unlabeled), retaining E0/EV. Sixteen randomized branches are required
per frozen prestate, exactly two per representation x signal cell.
"""
...
def validate_design(assignments: list[dict], frozen: dict[str, dict], config: dict) -> None:
    ...
    for branch in assignments:
        ...
        if cell not in CELLS:
            raise ValueError(f"branch {unit_id}: invalid A7 cell {cell!r}")
        ...
        if bool(source["initial_correct"]):
            raise ValueError(f"branch {unit_id}: A7 includes initially-correct prestate")
...
def render_message(chunks: dict[str, str], scaffold: str, encoding: str) -> str:
    state_body = (
        chunks["state_fields_body"] if encoding == "F" else chunks["state_prose_body"]
    )
    bodies = [
...
def build_messages(
    ...
    rendered = {
        key: render_message(chunks, *key)
        for key in (("L", "F"), ("L", "P"), ("U", "F"), ("U", "P"))
    }
    user_message = rendered[(scaffold, encoding)]
    state = semantic_state(source, signal_text, revision_instruction)
    bodies_payload = {
        "question_body": chunks["question_body"],
        "state_fields_body": chunks["state_fields_body"],
        "state_prose_body": chunks["state_prose_body"],
        "signal_body": chunks["signal_body"],
        "revision_body": chunks["revision_body"],
        "order": "QUESTION|PREVIOUS STATE|SIGNAL|REVISION REQUEST",
        "separator": SEPARATOR,
    }
    provenance = {
        "user_message": user_message,
        "user_message_sha256": sha256_text(user_message),
        "lf_user_message_sha256": sha256_text(rendered[("L", "F")]),
        "lp_user_message_sha256": sha256_text(rendered[("L", "P")]),
        "uf_user_message_sha256": sha256_text(rendered[("U", "F")]),
        "up_user_message_sha256": sha256_text(rendered[("U", "P")]),
        "semantic_state_json": canonical_json(state),
        "semantic_state_sha256": sha256_text(canonical_json(state)),
        "chunk_bodies_json": canonical_json(bodies_payload),
        "chunk_bodies_sha256": sha256_text(canonical_json(bodies_payload)),
        **chunks,
    }
    messages = [
...
def verify_factorial_bridge(source: dict, signals: dict, revision_instruction: str) -> None:
    for signal in ("E0", "EV"):
        signal_text = str(signals[signal])
        payloads = {}
        for scaffold, encoding in (("L", "F"), ("L", "P"), ("U", "F"), ("U", "P")):
            _, provenance = build_messages(
                source, scaffold, encoding, signal_text, revision_instruction
            )
            payloads[(scaffold, encoding)] = provenance

        semantic_hashes = {x["semantic_state_sha256"] for x in payloads.values()}
        semantic_jsons = {x["semantic_state_json"] for x in payloads.values()}
        chunk_hashes = {x["chunk_bodies_sha256"] for x in payloads.values()}
        chunk_jsons = {x["chunk_bodies_json"] for x in payloads.values()}
        if len(semantic_hashes) != 1 or len(semantic_jsons) != 1:
            raise ValueError(f"{source['id']} {signal}: representation changed semantic state")
        if len(chunk_hashes) != 1 or len(chunk_jsons) != 1:
            raise ValueError(f"{source['id']} {signal}: representation changed frozen chunk bodies")

        expected_key = {
            ("L", "F"): "lf_user_message_sha256",
            ("L", "P"): "lp_user_message_sha256",
            ("U", "F"): "uf_user_message_sha256",
            ("U", "P"): "up_user_message_sha256",
        }
        for rep, provenance in payloads.items():
            if provenance["user_message_sha256"] != provenance[expected_key[rep]]:
                raise ValueError(f"{source['id']} {signal}: {rep} provenance mismatch")
...
def main() -> int:
    ...
    parser.add_argument(
        "--a7-config", type=Path, default=Path("experiments/PILOT0_A7_CONFIG.json")
    )
    parser.add_argument(
        "--pilot-config",
        type=Path,
    ...
    parser.add_argument(
        "--model-dir", type=Path, default=Path("/kaggle/working/pilot0-qwen3-4b")
    )
    args = parser.parse_args()

    config = load_json(args.a7_config)
    if config.get("status") != "frozen before any A7 outcome generation":
        raise ValueError("A7 contract is not in the expected frozen pre-outcome state")
    ...
    for field, expected in frozen_values.items():
        if not same_float(inv[field], expected):
            raise ValueError(f"A7 invariant {field} differs from frozen Pilot 0 generation")
    if (
        inv["assistant_prefill"] != ASSISTANT_PREFILL
        or inv["choice_constraint"] != CHOICE_CONSTRAINT
    ):
        raise ValueError("A7 output interface differs from frozen Pilot 0 answer interface")
    ...
    for source in frozen_rows:
        if bool(source["initial_correct"]):
            continue
        verify_factorial_bridge(source, signals, revision_instruction)
    ...
    for index, branch in enumerate(assignments, 1):
        ...
        source = frozen[task_id]
        cell = str(branch["arm"])
        scaffold, encoding, signal = CELL_FACTORS[cell]
        signal_text = str(signals[signal])
        ...
        base_seed = stable_seed(
            int(inv["generation_base_seed"]), f"a7::{branch['id']}"
        )
        ...
        out.update(
            {
                "a7_cell": cell,
                "representation": scaffold + encoding,
                "scaffold": scaffold,
                "encoding": encoding,
                "signal": signal,
                "signal_text": signal_text,
                "factorial_bridge_verified": True,
                **provenance,
                ...
                "post_seed_rule": "stable_seed(generation_base_seed, 'a7::' + branch_id)",
                ...
                "post_generated_at_utc": datetime.now(timezone.utc).isoformat(
                    timespec="seconds"
                ),
            }
        )
    ...
    print(f"wrote {len(output)} A7 outcome branches", file=sys.stderr)
