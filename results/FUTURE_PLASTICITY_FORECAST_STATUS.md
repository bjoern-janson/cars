...
→ promoted to main after contract/smoke freeze
```
...
The active execution environment did not contain canonical MNIST and could not resolve external dataset hosts through its Python/network stack. The scientific config deliberately sets dataset download to `false` and requires the real MNIST benchmark rather than silently substituting another dataset.
